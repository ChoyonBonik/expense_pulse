...
class _EditExpenseScreenState extends ConsumerState<EditExpenseScreen> {
  ...
  Future<void> _loadExpense() async {
    final expense = await ref.read(appDatabaseProvider).getExpenseById(widget.expenseId);
  ...
  void _submit() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      final expenseCompanion = ExpensesCompanion(
        id: Value(widget.expenseId),
        amount: Value(amount),
        category: Value(_selectedCategory),
        date: Value(_selectedDate),
        note: Value(_noteController.text.isEmpty ? null : _noteController.text),
      );
      await ref.read(appDatabaseProvider).updateExpense(expenseCompanion);
      context.go('/');
    }
  }
